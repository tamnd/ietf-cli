...
describes what users get. Pick a format with `-o`, or let ietf choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ietf <command> -o table   # aligned columns for reading
ietf <command> -o jsonl   # one JSON object per line, for piping
ietf <command> -o json    # a single JSON array
ietf <command> -o csv     # spreadsheet friendly
ietf <command> -o tsv     # tab-separated
ietf <command> -o url     # just the URL column
ietf <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ietf <command> --fields id,title,url
```
...
```bash
ietf <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ietf <command>            # a table, because this is a terminal
ietf <command> | wc -l    # JSONL, because this is a pipe
```
