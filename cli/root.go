// Package cli assembles the ietf command tree from the ietf
...
// NewApp assembles the kit application from the ietf domain. The
...
//
// To add a command, declare it in ietf/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := ietf.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(ietf.Domain{}).Register(app)
